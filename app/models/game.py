...
class Game:
    ...
    def get_legal_moves(self, square):
        """Legal moves for the piece at square (must belong to current_turn)."""
        return self._get_legal_moves_for(square, self.current_turn)

    def _get_legal_moves_for(self, square, color):
        """
        Legal moves for the piece at square belonging to color, regardless of
        current_turn. Handles check-filtering, castling through-check, and en
        passant capture simulation correctly.
        """
        ...
        # Needed to validate castling (can't castle while in check)
        king_currently_in_check = (
            self.board.is_square_attacked(square, opponent_color) if is_king else False
        )

        legal = []

        for move_sq in piece.get_legal_moves(self.board):
            # ── Castling extra rules ───────────────────────────────────────────
            if is_king and abs(square[1] - move_sq[1]) == 2:
                if king_currently_in_check:
                    continue  # Can't castle out of check
                mid_col = (square[1] + move_sq[1]) // 2
                if self.board.is_square_attacked((square[0], mid_col), opponent_color):
                    continue  # Can't castle through check

            # ── Simulate the move ──────────────────────────────────────────────
            captured = self.board.get_piece(move_sq)
            ep_sq = None
            ep_piece = None

            # En passant: pawn moves diagonally to empty square
            if isinstance(piece, Pawn) and captured is None and square[1] != move_sq[1]:
                ep_sq = (square[0], move_sq[1])
                ep_piece = self.board.get_piece(ep_sq)
                self.board.set_piece(ep_sq, None)
            ...
            self.board.set_piece(move_sq, captured)
            if ep_sq:
                self.board.set_piece(ep_sq, ep_piece)
        ...
        for row in range(8):
            for col in range(8):
                ...
                if piece and piece.color == color:
                    if self._get_legal_moves_for((row, col), color):
                        return False
        return True
    ...
    def _generate_notation(self, from_sq, to_sq, piece, captured):
        ...
        return piece_char + ('x' if captured else '') + target
    ...
    def to_dict(self):
        return {
            ...
            'last_move': (
                self.move_history.get_last_move().to_dict()
                if self.move_history.get_last_move() else None
            ),
        }

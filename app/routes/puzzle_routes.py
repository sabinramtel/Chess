...
def get_next_puzzle():
    ...
    puzzle, stats = _ctrl().get_next_puzzle(user_id)
    if not puzzle:
...
def submit_attempt():
    ...
    result = _ctrl().check_move(user_id, int(puzzle_id), move, move_index)
...
def get_stats():
    ...
    return jsonify(_ctrl().get_stats(user_id))
